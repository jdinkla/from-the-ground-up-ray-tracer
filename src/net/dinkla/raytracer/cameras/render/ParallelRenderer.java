...
public class ParallelRenderer implements IRenderer {
    ...
    protected void createWorkers(IFilm film) {
        ...
        if (res.getVres() % vertFactor == 0) {
            worker = new Worker[numThreads];
            int yStep = film.getResolution().getVres() / vertFactor;
            for (int i = 0; i < numThreads / 4; i++) {
                worker[4*i]   = new Worker(0,            1* res.getHres() /4, i * yStep, (i + 1) * yStep);
                worker[4*i+1] = new Worker(1* res.getHres() /4, 2* res.getHres() /4, i * yStep, (i + 1) * yStep);
                worker[4*i+2] = new Worker(2* res.getHres() /4, 3* res.getHres() /4, i * yStep, (i + 1) * yStep);
                worker[4*i+3] = new Worker(3* res.getHres() /4, 4* res.getHres() /4, i * yStep, (i + 1) * yStep);
            }
        } else {
