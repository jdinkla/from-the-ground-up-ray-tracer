...
public class Orthographic extends AbstractLens {
    ...
    public Ray getRaySampled(int r, int c, Point2D sp) {
        int x = (int) (viewPlane.size * (c - 0.5 * (viewPlane.resolution.hres - 1) + sp.getX()));
        int y = (int) (viewPlane.size * (r - 0.5 * (viewPlane.resolution.vres - 1) + sp.getY()));
        Ray ray = new Ray(new Point3D(x, y, zw), Vector3D.Companion.getBACK());
        return ray;
    }
